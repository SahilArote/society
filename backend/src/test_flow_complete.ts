...
async function runEndToEndVerification() {
  ...
  const guardLoginJson: any = await guardLoginRes.json();
  ...
  fs.writeFileSync(dummyPhotoPath, Buffer.from(tinyJpegBase64, 'base64'));
  ...
  const reqListJson: any = await reqListRes.json();
  const myRequest = reqListJson.data?.find((r: any) => r.id === requestId);
  ...
  const otherResidentJson: any = await otherResidentOtpRes.json();
  ...
  const adminActivityJson: any = await adminActivityRes.json();
  const adminItem = adminActivityJson.data?.find((a: any) => a.id === requestId);

...
async function seedAdminData() {
    ...
    const [vCountRows] = await pool.query('SELECT COUNT(*) as cnt FROM visitor_requests WHERE society_id = ?', [societyId]);
    ...
    if (currentCount < 10) {
        ...
        const [flatRows] = await pool.query('SELECT id, flat_number, resident_id FROM flats WHERE society_id = ? LIMIT 10', [societyId]);

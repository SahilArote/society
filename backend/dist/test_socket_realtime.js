...
async function testRealtimeSockets() {
    ...
    const resJson = await resAuth.json();
    ...
    const guardJson = await guardAuth.json();
    const guardToken = guardJson.data.token;
    console.log('--- 2. Connecting Realtime WebSockets ---');
    const residentSocket = (0, socket_io_client_1.io)(socketUrl, { auth: { token: residentToken } });
    const adminSocket = (0, socket_io_client_1.io)(socketUrl, { auth: { token: adminToken } });
    ...
    residentSocket.on('visitor:request_created', (data) => {
    ...
    adminSocket.on('admin:visitor_activity', (data) => {
    ...
    residentSocket.on('visitor:request_updated', (data) => {
